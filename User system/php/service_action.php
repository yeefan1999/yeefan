...
<?php
	...
	$service = mysqli_query($conn, "select * from service where service_id = '$id'");
	...
	if(isset($_POST["addtocart"]))
	{
		...
		if(mysqli_num_rows($cart)==1)
		{
			...
			$eventid=$cart3['event_id'];
					if($cart3['service_id']==0 )
					{

					mysqli_query($conn,"update event set service_id='$id1' where event_id='$eventid'");
					$cart=0;
					}
					
					else if($cart3['service_id2']==0)
					{
					
					if($id1==$cart3['service_id']||$id1==$cart3['service_id2']||$id1==$cart3['service_id3'])
					{
						$cart=2;
					}
					else
					{
					mysqli_query($conn,"update event set service_id2='$id1' where event_id='$eventid'");
					$cart=0;
					}
					}
					
					else if($cart3['service_id3']==0)
					{
					if($id1==$cart3['service_id']||$id1==$cart3['service_id2']||$id1==$cart3['service_id3'])
					{
						$cart=2;
					...
					{
					mysqli_query($conn,"update event set service_id3='$id1' where event_id='$eventid'");
					$cart=0;
					}
					}
				
				
					else if ($cart3['service_id']!=0 && $cart3['service_id2'] !=0 && $cart3['service_id3']!=0)
					{
						$cart=3;
					}
			 ...
			 <div class="top-sing">
				  <div class="col-md-7 single-top">	
					 <div class="flexslider">
							  <ul class="slides">
								<li data-thumb="src="../../Admin system/pages/<?php echo $service1["service_image"]; ?>" ">
									<div class="thumb-image"> <img src="../../Admin system/pages/<?php echo $service1["service_image"]; ?>"  data-imagezoom="true" class="img-responsive" alt=""/> </div>
								</li>
			     ...
			     <div class="col-md-5 single-top-in simpleCart_shelfItem" style="float:right; margin-top:-280px;">
					...
					<h4><b><?php echo $service1["service_name"]; ?></b></h4>						 
							<h5 class="item_price">RM <?php echo $service1["service_price"]; ?></h5>							
						
							<div class="prdt-info-grid">
							<p>Description:</p>
								 <?php echo $service1["service_description"]; ?>
							</div>
							<!--
							<div class="check">
				 ...
				 $feedback=mysqli_query($conn,"select * from feedback where service1_id='$serviceid' or service2_id='$serviceid' or service3_id='$serviceid'");
				 
				if(mysqli_num_rows($feedback)!=0)
				 {
				$q1=mysqli_query($conn,"select * from feedback where service1_id='$serviceid';");
				$q11=mysqli_fetch_assoc($q1);
				
				$sum1=$q11['service1_rating'];
				
				$q2=mysqli_query($conn,"select * from feedback where service2_id='$serviceid';");
				$q22=mysqli_fetch_assoc($q2);
				
				$sum2=$q22['service2_rating'];
				
				$q3=mysqli_query($conn,"select * from feedback where service3_id='$serviceid';");
				$q33=mysqli_fetch_assoc($q3);
				
				$sum3=$q33['service3_rating'];
				
				$a1=mysqli_query($conn,"select count(feedback_id) as cout from feedback where service1_id='$serviceid' or service2_id='$serviceid' or service3_id='$serviceid';");
				$a11=mysqli_fetch_assoc($a1);
				$count=$a11['cout'];
				...
				$avg=number_format($avg, 1);
				 ...
				 {		
					...
					<?php 
					if($f1['service1_id']==$serviceid || $f1['service2_id']==$serviceid || $f1['service3_id']==$serviceid)
					{
					echo $final['c_name'];?> <span style="text-transform:none">said:</span><?php }?>
					</td>
					</tr>
					<tr>
					<td >
					...
					<?php 
					if($f1['service1_id']==$serviceid)
					{
					echo $f1['service1_feedback'];
					}
					if($f1['service2_id']==$serviceid)
					{
					echo $f1['service2_feedback'];
					}
					if($f1['service3_id']==$serviceid)
					{
					echo $f1['service3_feedback'];
					}
					?>
					</td>
					</tr>
				 ...
				 else
				 {
					 ...
					 <h2> This service has no reviews yet.</h2>
					 <?php
				 }
				 ?>
			 </div>
...
if(isset($_POST["addtocart"]))
	{
		...
		{
			echo '<script>swal("Error", "This product is already in your cart. Please check at your cart.", "error");</script>';
		
		}
		else if($cart == 3)
		{
			echo '<script>swal("Error", "You can only have additional services at most 3 per event.", "error");</script>';
