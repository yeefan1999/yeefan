    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
        <div class="content-wrapper">
		  ...
		  $exe1=mysqli_query($conn,"select package_id,count(package_id) from event group by package_id order by count(package_id) desc limit 1");
		  $exe=mysqli_fetch_assoc($exe1);
		  
		  $pexeid=$exe['package_id'];
		  $pexe=mysqli_query($conn,"select * from package where package_id ='$pexeid'");
		  $pexee=mysqli_fetch_assoc($pexe);
            ...
            <div class="col-md-4 stretch-card grid-margin">
              <div class="card bg-gradient-danger card-img-holder text-white">
                <div class="card-body">
                  ...
                  <h4 class="font-weight-normal mb-3">Most Popular Package
                    <i class="mdi mdi-chart-line mdi-24px float-right"></i>
                  </h4>
				  <br><br>
                  <h2 class="mb-5" style="text-transform:uppercase;"><?php echo $pexee['package_name']?></h2>
                </div>
              </div>
            </div>
			<?php
		  ...
		  $texe1=mysqli_query($conn,"select theme_id,count(theme_id) from event group by theme_id order by count(theme_id) desc limit 1");
		  $texe=mysqli_fetch_assoc($texe1);
		  
		  $texeid=$texe['theme_id'];
		  $texe=mysqli_query($conn,"select * from theme where theme_id ='$texeid'");
		  $texee=mysqli_fetch_assoc($texe);
		  ...
		  ?>
            <div class="col-md-4 stretch-card grid-margin">
              <div class="card bg-gradient-info card-img-holder text-white">
                <div class="card-body">
                  <img src="../images/dashboard/circle.svg" class="card-img-absolute" alt="circle-image"/>                  
                  <h4 class="font-weight-normal mb-3">Hottest Theme
                    <i class="mdi mdi-bookmark-outline mdi-24px float-right"></i>
                  </h4>
				   <br><br>
                  <h2 class="mb-5" style="text-transform:uppercase;"><?php echo $texee['theme_name']?></h2>
                </div>
              </div>
            </div>
			<?php
		  ...
		  $sexe1=mysqli_query($conn,"select service_id,count(service_id) from event group by service_id order by count(service_id) desc limit 1");
		  $sexe=mysqli_fetch_assoc($sexe1);
		  
		  $sexe2=mysqli_query($conn,"select service_id2,count(service_id2) from event group by service_id2 order by count(service_id2) desc limit 1");
		  $sexe22=mysqli_fetch_assoc($sexe2);
		  
		  $sexe3=mysqli_query($conn,"select service_id3,count(service_id3) from event group by service_id3 order by count(service_id3) desc limit 1");
		  $sexe33=mysqli_fetch_assoc($sexe3);
		  
		  $ser1=$sexe['count(service_id)'];
		  $ser2=$sexe22['count(service_id2)'];
		  $ser3=$sexe33['count(service_id3)'];
		  $win=0;
		  $serid=0;
		  if($ser1>=$ser2 && $ser1>=$ser3)
		  {
			  $win=$ser1;
			  $serid=$sexe['service_id'];
		  }
		  else if($ser2>=$ser1&&$ser2>=$ser3)
		  {
			  $win=$ser2;
			  $serid=$sexe22['service_id2'];
		  }
		  else if ($ser3>=$ser1&&$ser3>=$ser2)
		  {
			  $win=$ser3;
			  $serid=$sexe33['service_id3'];
		  }
		  else
		  {
			  $win="no";
		  ...
		  ?>
            <div class="col-md-4 stretch-card grid-margin">
              <div class="card bg-gradient-success card-img-holder text-white">
                <div class="card-body">
                  <img src="../images/dashboard/circle.svg" class="card-img-absolute" alt="circle-image"/>                                    
                  <h4 class="font-weight-normal mb-3">Best Sales of Service
                    <i class="mdi mdi-diamond mdi-24px float-right"></i>
                  </h4>
				  <br><br>
                  <h2 class="mb-5" style="text-transform:uppercase;">
				  
				  <?php 
				  if($win=="no" || $serid==0)
				  {
				  echo "No analysis now";
				  }
				  else
				  {	  $sexeid=$serid;
					  $ssexe=mysqli_query($conn,"select * from service where service_id ='$sexeid'");
					  $sexee1=mysqli_fetch_assoc($ssexe);
					  $sname=$sexee1['service_name'];
					  echo $sname;
...
<?php
	...
	$all=mysqli_query($conn,"select count(event_id) from event");
	$all1=mysqli_fetch_assoc($all);
	
	$all11=$all1['count(event_id)'];
	
	$pall=mysqli_query($conn,"select * from event");
	$w=0;
		$c=0;
		$b=0;
	while($pall1=mysqli_fetch_assoc($pall))
	{
		$pacid=$pall1['package_id'];
		$ptype=mysqli_query($conn,"select * from package where package_id='$pacid'");
		
		$ptype1=mysqli_fetch_assoc($ptype);
		$ptype11=$ptype1['package_type'];
		...
		else if($ptype11=='Corporate')
		{
			$c=$c+1;
		}
		else if($ptype11=='Birthday')
		{
			$b=$b+1;
		}
	}
	...
	$wa1=number_format($wa, 2);
	$ba1=number_format($ba, 2);
	$ca1=number_format($ca, 2);
	
    $dataPoints = array(
        array("y" => $wa1, "legendText" => "Wedding", "label" => "Wedding"),
        array("y" => $ba1, "legendText" => "Corporate", "label" => "Corporate"),
        array("y" => $ca1, "legendText" => "Birthday", "label" => "Birthday"),
...
<script type="text/javascript">
    $(function () {
        var chart = new CanvasJS.Chart("chartContainer", {
            title: {
                text: "Event Held Statistic Analysis"
            },
