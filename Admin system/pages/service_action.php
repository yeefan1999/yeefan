...
<?php
	...
	$ser = mysqli_query($conn, "select * from service where service_id = '$id'");
...
<?php
	...
	if(isset($_POST["service_change"]))
		{
			$id11 = $ser1["service_id"];
			$name = $_POST["servicename"];
			$type = $_POST["servicetype"];
			$price = $_POST["serviceprice"];
			$details = $_POST["servicedesc"];
			$check = "";
			$check1="";
			...
			$checkname=mysqli_query($conn,"select * from service where service_name='$name1'");
			$checkname1=mysqli_fetch_assoc($checkname);
			
			$checkname2=mysqli_query($conn,"select * from service where service_id='$id'");
			$checkname3=mysqli_fetch_assoc($checkname2);
			$namenow=$checkname3['service_name'];
			
			$check=strtolower($checkname1['service_name']);
			
			$namechange = $_POST["servicename"];
			
			if(mysqli_num_rows($checkname)==0||$namechange==$namenow)
			{
			...
			if($check1=='1')
			{
					...
					$sql =  "update service set service_name='$name',service_type='$type', service_price='$price',
					service_description='$details',service_image='$target_dir$profilename' where service_id='$id11'";
			...
			{
				$sql =  "update service set service_name='$name',service_type='$type', service_price='$price',
					service_description='$details' where service_id='$id11'";
      ...
      <div class="main-panel">
        <div class="content-wrapper">
		  ...
		  <br><br>
              ...
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Edit Service Form</h4>
                  <form class="form-sample" enctype='multipart/form-data' name="chgserviceform" autocomplete = "off" method = "post" action = "">
				<br>
					<br>
                      ...
                      <div class="col-md-6">
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
							...
							value="<?php echo $ser1["service_name"]; ?>"/>
                          </div>
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <select class="form-control" id = "servicetype" name="servicetype">
                              <option value="Clown" <?php if($ser1["service_type"] == "Clown"){ echo "selected"; } ?>>Clown</option>
                              <option value="Photobooth" <?php if($ser1["service_type"] == "Photobooth"){ echo "selected"; }?>>Photobooth</option>
                              <option value="Emcee" <?php if($ser1["service_type"] == "Emcee"){ echo "selected"; }?>>Emcee</option>
							  <option value="Liveband" <?php if($ser1["service_type"] == "Liveband"){ echo "selected"; }?>>Liveband</option>
                              <option value="Other Product" <?php if($ser1["service_type"] == "Other Product"){ echo "selected"; }?>>Other Product</option>
                            </select>
                          </div>
                        </div>
                      </div>
                   ...
                   <div class="row">
                      <div class="col-md-6">
                        <div class="form-group row">
                          <label class="col-sm-3 col-form-label">Service Price (RM) </label>
                          <div class="col-sm-9">
                            <input type="text" class="form-control" id = "serviceprice" name="serviceprice" value="<?php echo $ser1["service_price"]; ?>" />
                          </div>
                        </div>
                      </div>
                      <div class="col-md-12">
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                              <textarea class="form-control" id="servicedesc" name="servicedesc" ><?php echo $ser1["service_description"]; ?>"</textarea>
					...
					<br>
					<button type="submit" name="service_change" id="service_change" class="btn btn-gradient-primary mr-2">Change</button>
                    <a href="service_list.php"><button class="btn btn-light">Cancel</button></a>
					<br>
					</form>
                </div>
              </div>
...
<?php
	...
	{
		if($check=='1')
		{ echo '<script>swal("Success", "Service details is succesfully updated!", "success");</script>';
		$check="";
		}
	
		else if ($check=='0')
		{echo '<script>swal("Error", "Service updated failed. Try again later.", "error");</script>';
		$check="";}
		
		else if ($check=='3')
		{echo '<script>swal("Error", "Service name repeated. Change a name.", "error");</script>';
		$check="";}
		else if ($check=='4')
